		...
		if(message.substr(0, 1) == "!") {
			...
			switch(command) {
				...
				case 'rule':
					...
					case 'house':
						if(text) {
							text = text.toLowerCase();	// Normalize usernames
							// Look up a specific user's house
							Users.house(text, function(house) {
								
								if(house) {
									var line = text + " is a proud member of house: " + house;
									response.push(line);
									callback(response);
								}
								else {
									var line = text + " is a muggle! How IMBARASSING";
									response.push(line);
									callback(response);
								}
							});
							...
							// Look up my own house
							Users.house(userstate.username, function(house) {
								if(house) {
									var line = userstate.username + " is a proud member of house: " + house;
									response.push(line);
									callback(response);
								}
								else {
									var line = userstate.username + " is a muggle! How IMBARASSING";
									response.push(line);
									callback(response);
								}
							})
