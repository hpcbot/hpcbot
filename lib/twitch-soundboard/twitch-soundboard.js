...
var start = function start(_options) {
    ...
    io.on('connection', function (socket) {
        socket.on('gift', function() {
          options.events.emit('overlays:gift:show');
        });
        socket.on('muggle', function() {
          options.events.emit('overlays:muggle:show');
        });
        socket.on('sac', function() {
          options.events.emit('overlays:sac:show');
        });
        socket.on('powermove', function() {
          options.events.emit('overlays:powermove:show');
        });
        socket.on('monster', function() {
          options.events.emit('overlays:monster:show');
        });
        socket.on('smoke', function() {
          options.events.emit('overlays:smoke:show');
        });
        socket.on('kia', function() {
          options.events.emit('overlays:kia:show');
        });
        socket.on('cheese', function() {
          options.events.emit('overlays:cheese:show');
        });
        socket.on('hpcwins', function() {
          options.events.emit('overlays:hpcwins:show');
        });
        socket.on('tworax', function() {
          options.events.emit('overlays:tworax:show');
        });
        socket.on('polo', function() {
          options.events.emit('overlays:polo:show');
        });
        socket.on('first', function() {
          options.events.emit('overlays:first:show');
        });
        socket.on('buyback', function() {
          options.events.emit('overlays:buyback:show');
        });
        socket.on('open', function() {
          options.events.emit('overlays:open:show');
        });
        socket.on('close', function() {
          options.events.emit('overlays:close:show');
        });
        ...
        socket.on('quidditch', function() {
          options.events.emit('overlays:quidditch:show');
        });
    });
