...
var start = function start(_options) {
    ...
    options.events.emit('commands:add', commands);
...
var onConnect = function(socket) {
  ...
  socket.on('endSong', function(videoId) {
    // Prevents two users from ending a song at the same time
    options.events.emit('log:info', 'music:endSong', {videoId: videoId, socketId: socket.id, host: host});
...
var playSong = function(song) {
  ...
  Playlist.play(song, function(err, success) {
    options.events.emit('log:info', 'music:playSong', {song: song});
    update();
  });
}
  ...
  if(url) {
    if(source != 'chat' && url.list) {
            ...
            Playlist.add(song, function(err, repsonse) {
              if(!err) {
                ...
                if(list.length == songs.length) {
                  ...
                  options.events.emit('log:info', 'music:addlist', {list: list});
                  update();
                }
              }
            });
      ...
      Playlist.addAfter(id, lastRequest, function(err, success) {
        if(err) {
          error = err;
          options.events.emit('log:error', 'music:addlist', {err: error, list: list});
        }
        ...
        Song.add(id, function(err, metadata) {
          if(err) {
            error = err;
            options.events.emit('log:error', 'music:addlist', {err: error, list: list, song: id});
          }
          ...
          if(error) {
            ...
            options.events.emit('log:info', 'music:addsong', {song: id});
          }
          update();
        })
      });
...
var removeSong = function(song) {
  ...
  Playlist.remove(song, function(err, success) {
    options.events.emit('log:info', 'music:removesong', {song: song});
    update();
  });
}
...
var shuffle = function() {
  Playlist.shuffle(function(err, success) {
    options.events.emit('log:info', 'music:shuffle', {success: success});
    update();
  });
}
...
var reorder = function(start, end) {
  Playlist.reorder(start, end, function(err, success) {
    options.events.emit('log:info', 'music:reorder', {start: start, end: end});
    update();
  });
}
...
var addClient = function(id, user) {
  // a new client has connected
  if(user) {
    user.id = id;
    clients.push(user); // client has twitch data
    options.events.emit('log:info', 'music:connected', {user: user, id: id, clients: clients});
  }
}
...
var removeClient = function(id) {
  ...
  for(var i=0; i < clients.length; i++) {
    if(clients[i].id == id) {
      clients.splice(i, 1); // Remove client from the array
      options.events.emit('log:info', 'music:disconnected', {id: id, clients: clients});
    }
  }
}
