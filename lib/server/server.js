...
var start = function(_options) {
  ...
  io.on('connection', function (socket) {
    ...
    if(options.soundboard) {
      /* Initialize soundboard listeners */
      options.soundboard.onConnect(socket);
    }
  });

  if(options.music) {
    options.events.on('music:state', musicState);
    console.log(`[Music] Initialized`)
  }

  if(options.soundboard) {
    options.events.on('soundboard:state', soundboardState);
...
var startServer = function() {
  ...
  if(options.music && options.music.server) {
    app.use(options.music.server.route, express.static(options.music.server.static));
  }

  // Configure soundboard routes and static dir
  if(options.soundboard && options.soundboard.server) {
    app.use(options.soundboard.server.route, express.static(options.soundboard.server.static))
    app.get(options.soundboard.server.route, auth(), function(req, res, next) {
      res.render(options.soundboard.server.index, {css: options.soundboard.server.css})
    })

    // options.events.on('soundboard:css', (file) => {
    //   if(file) {
    //
    //   }
    // })
...
var musicState = function(_state) {
  io.sockets.emit('music:state', _state); // Send an update to all connected clients
}

var soundboardState = function(_state) {
  io.sockets.emit('soundboard:state', _state);
}
...
var auth = function() {
  ...
  return function (req, res, next) {
    if (req.isAuthenticated()) {
      ...
      return next();  // HACK - Bypass twitch login
      // User isn't authenticated, send them to auth
      // req.session.returnTo = req.url  // Send users back to where they came from after auth'ing
