...
var	overlay = {
  ...
  event: 'stream:house',  						// The event that shows your overlay (required)
  view: path.join(__dirname, 'views/house.pug'),  // The view you want to be rendered (required)
  selector: '.sortinghat',  						// The selector to select your template (optional: default to a class w/ the  .name of the overlay)
...
var start = function start(_eventbus, _User) {
	...
	eventbus.on('house:test', testHouse); // Test the sorting hat overlay
};
			...
			User.create(username, function(err, user) {
				User.setHouse(username, function (err, house) {
					if(!err) {
						...
						eventbus.emit('stream:house', payload);
					}
				});
			});
		}
...
var testHouse = function testHouse(username) {
	...
	// What should we show on stream?
 	var payload = assemblePayload('Testy McTesterson', house);
	eventbus.emit('stream:house', payload);
};
