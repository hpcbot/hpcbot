...
var spook = function spook(username, parameters) {
  ...
  var value = 5;  // Add or subtract 5 points to a house
	var abbrhouse;
	var hweenoverlay = 'hween';

  var houseNumber = Math.floor(Math.random() * (3+1)); // Generate a random number between 0-3
  ...
  switch(houseNumber) {
	...
	var addOrSub = Math.floor(Math.random() * (1+1)); // Generate a random number between 0-1
    ...
    case 1:
      // Subtract points from house
        ...
        if(!err && data) {
					eventbus.emit(hweenoverlay);  // Show the halloween overlay
					setTimeout(function() {
						response.push(strings.cup.subtracted + 5 + strings.cup.points_from + house);
						eventbus.emit('twitch:say', response);
					}, 55000);
        } else {
