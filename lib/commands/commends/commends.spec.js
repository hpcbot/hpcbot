...
describe('Commends', function() {
	...
	describe('Chat Triggers', function() {
		it('Contains settings for !commends', function() {
			assert.notEqual(Commends.triggers, null);
			assert.equal(Commends.triggers[0].name, 'commends');
			assert.equal(Commends.triggers[0].type, 'chat');
			assert.equal(Commends.triggers[0].whitelist, null);
			assert.equal(Commends.triggers[0].event, 'commends:get');
		});
		it('Contains settings for !setcommends', function() {
			assert.notEqual(Commends.triggers, null);
			assert.equal(Commends.triggers[1].name, 'setcommends');
			assert.equal(Commends.triggers[1].type, 'whisper');
			assert.equal(Commends.triggers[1].whitelist, true);
			assert.equal(Commends.triggers[1].event, 'commends:set');
		});
	});
