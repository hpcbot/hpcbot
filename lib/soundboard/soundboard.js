...
class Soundboard {
  constructor(_options) {
    ...
    options.events.on('soundboard:add', this.add)
    ...
    io.on('connection', function (socket) {
      /*
        socket.on('gift', function() {
          options.events.emit('overlay:gift:show');
        });
        socket.on('muggle', function() {
          options.events.emit('overlay:muggle:show');
        });
        socket.on('sac', function() {
          options.events.emit('overlay:sacrifice:show');
        });
        socket.on('powermove', function() {
          options.events.emit('overlay:powermove:show');
        });
        socket.on('monster', function() {
          options.events.emit('overlay:monster:show');
        });
        socket.on('smoke', function() {
          options.events.emit('overlay:smoke:show');
        });
        socket.on('kia', function() {
          options.events.emit('overlay:kia:show');
        });
        socket.on('cheese', function() {
          options.events.emit('overlay:cheese:show');
        });
        socket.on('hpcwins', function() {
          options.events.emit('overlay:hpcwins:show');
        });
        socket.on('tworax', function() {
          options.events.emit('overlay:tworax:show');
        });
        socket.on('polo', function() {
          options.events.emit('overlay:polo:show');
        });
        socket.on('first', function() {
          options.events.emit('overlay:first:show');
        });
        socket.on('buyback', function() {
          options.events.emit('overlay:buyback:show');
        });
        socket.on('open', function() {
          options.events.emit('overlay:open:show');
        });
        socket.on('close', function() {
          options.events.emit('overlay:close:show');
        });
        ...
        socket.on('quidditch', function() {
          options.events.emit('overlay:quidditch:show');
        });
        socket.on('xmas', function() {
          options.events.emit('xmas:start');
        });
        socket.on('3runes', function() {
          options.events.emit('overlay:3runes:show');
        });
        socket.on('picks', function() {
          options.events.emit('overlay:picks:show');
        });
        */
    });
  }
