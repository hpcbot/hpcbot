...
exports = module.exports = class Hpcbot {
  ...
  constructor(options) {
    ...
    // Goblin Gold
    var Gold = require('./command/gold');
    Gold.start(eventbus, Resource, Channel);
    commands.push(Gold);

    // !setcommends / !commends
    var Commends = require('./command/commends');
    Commends.start(eventbus, User);
    commands.push(Commends);

    // !rules
    var Rules = require('./command/rules');
    Rules.start(eventbus);
    commands.push(Rules);

    // !house / !sortinghat
    var House = require('./command/house');
    House.start(eventbus, User);
    this.overlays.add(House.overlay);
    commands.push(House);
