...
var listenToCommands = function(commands) {
	/* Add a command to the list
	Input: {command} or {commands}
	Output: none */

	options.events.on('commands:add', function(commands) {
		add(commands);
	});
}
...
var parse = function parse(userstate, message, self, callback) {
	...
	if(message.substr(0, 1) == "!") {
		...
		command = command.toLowerCase();	// Convert all commands to lowercase
